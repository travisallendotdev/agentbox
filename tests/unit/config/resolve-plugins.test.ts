...
import { resolvePlugin, resolveAllPlugins } from "../../../src/config/resolve-plugins.ts";
...
test("resolves a marketplace-qualified plugin", async () => {
  ...
  expect(r.path).toBe(join(SKILLS_HOME, "plugins/cache/foo-marketplace/superpowers/5.0.0"));
});
...
test("resolves an absolute path as a local plugin", async () => {
  const explicit = join(SKILLS_HOME, "plugins/cache/foo-marketplace/superpowers/5.0.0");
...
test("missing bare plugin errors", async () => {
  await expect(resolvePlugin("does-not-exist")).rejects.toThrow(/does-not-exist/);
});

test("picks the lexically-greatest version when multiple are present", async () => {
  const dir = join(SKILLS_HOME, "plugins/cache/foo-marketplace/superpowers/5.0.1");
...
test("resolveAllPlugins returns array preserving order", async () => {
  const r = await resolveAllPlugins(["foo-marketplace:superpowers", "superpowers"]);
