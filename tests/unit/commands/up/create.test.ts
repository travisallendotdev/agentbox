...
test("expands bare variant name to docker/sandbox-templates", () => {
  expect(resolveTemplateRef("claude-code-docker")).toBe(
    "docker.io/docker/sandbox-templates:claude-code-docker",
  );
  expect(resolveTemplateRef("claude-code")).toBe(
    "docker.io/docker/sandbox-templates:claude-code",
  );
});

test("passes through full OCI references unchanged", () => {
  expect(resolveTemplateRef("docker.io/myorg/foo:v1")).toBe(
    "docker.io/myorg/foo:v1",
  );
