...
export async function cloneGitRepos(sandboxName: string, repos: ResolvedRepo[]): Promise<void> {
  const parent = homePaths().repoParentDir(sandboxName);
  for (const r of repos) {
    if (r.source !== "git") continue;
    const dest = r.place === "vm"
      ? `/home/agent/repos/${r.name}`
      : `${parent}/${r.name}`;
    ...
    if (result.exitCode !== 0) {
      throw new AgentboxError(`git clone failed for ${r.url}: ${result.stderr.trim()}`, {
