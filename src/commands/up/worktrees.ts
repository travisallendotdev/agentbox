...
export async function assertWorktreesClean(
  sandboxName: string,
  repos: ResolvedRepo[],
): Promise<void> {
...
export async function createHostWorktrees(
  sandboxName: string,
  repos: ResolvedRepo[],
): Promise<void> {
  const parent = homePaths().repoParentDir(sandboxName);
  ...
  try {
    ...
    for (const c of created.reverse()) {
      try {
        await removeWorktree(c.repoDir, c.worktreePath, { force: true });
      } catch (cleanupErr) {
        console.warn(
          `agentbox: rollback warning: failed to remove worktree ${c.worktreePath}: ${(cleanupErr as Error).message}`,
        );
      }
    }
